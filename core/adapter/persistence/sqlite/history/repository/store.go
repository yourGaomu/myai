...
func (s *Store) HasBaseline(ctx context.Context, workspace string) (bool, error) {
	var exists int
...
func (s *Store) LoadBaseline(ctx context.Context, workspace string) (map[string]domainhistory.FileSnapshot, error) {
	...
	for rows.Next() {
		var item domainhistory.FileSnapshot
		var hashText string
		var mode int64
		var binary int
		var tooLarge int
...
func (s *Store) ReplaceBaseline(ctx context.Context, workspace string, files map[string]domainhistory.FileSnapshot) error {
...
func (s *Store) SaveCheckpoint(ctx context.Context, checkpoint domainhistory.Checkpoint, changes []domainhistory.FileChange) (string, error) {
	if len(changes) == 0 {
		return "", nil
	}
...
func (s *Store) ListCheckpoints(ctx context.Context, workspace string, limit int) ([]domainhistory.CheckpointSummary, error) {
	...
	result := make([]domainhistory.CheckpointSummary, 0)
	for rows.Next() {
		var item domainhistory.CheckpointSummary
...
func (s *Store) LoadCheckpointChanges(ctx context.Context, workspace string, checkpointID string) ([]domainhistory.StoredFileChange, error) {
	...
	result := make([]domainhistory.StoredFileChange, 0)
	for rows.Next() {
		var item domainhistory.StoredFileChange
...
func (s *Store) ensureFileChangeColumns(ctx context.Context) error {
...
func (s *Store) fileChangeColumns(ctx context.Context) (map[string]bool, error) {
...
func nullableContent(file domainhistory.FileSnapshot) any {
	if !file.Available {
		return nil
	}
	return file.Content
...
func nullableHash(file *domainhistory.FileSnapshot) any {
	if file == nil {
		return nil
	}
...
func nullableSnapshotContent(file *domainhistory.FileSnapshot) any {
...
func nullableSnapshotSize(file *domainhistory.FileSnapshot) any {
	if file == nil {
		return nil
	}
	return file.Size
...
func nullableSnapshotMode(file *domainhistory.FileSnapshot) any {
	if file == nil {
		return nil
	}
...
func snapshotAvailable(file *domainhistory.FileSnapshot) bool {
	return file != nil && file.Available
}

func storedSnapshot(path string, hashText sql.NullString, content []byte, size sql.NullInt64, mode sql.NullInt64, available bool) (*domainhistory.FileSnapshot, error) {
	if !hashText.Valid {
		return nil, nil
	}
